...
npx wrangler pages project create "${PROJECT_NAME}" --production-branch main 2>&1   | grep -v 'already exists' || true

# Deploy
npx wrangler pages deploy "${PREVIEW_DIR}/"     --project-name "${PROJECT_NAME}"     --branch main     --commit-dirty true 2>&1

# Add CNAME {slug}.coreflux.studio -> {slug}.pages.dev (proxied, idempotent)
curl -s -X POST "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records"     -H "Authorization: Bearer $CF_TOKEN"     -H "Content-Type: application/json"     -d "{
      \"type\": \"CNAME\",
      \"name\": \"${SLUG}\",
...
curl -s -X POST   "https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/pages/projects/${PROJECT_NAME}/domains"   -H "Authorization: Bearer $CF_TOKEN"   -H "Content-Type: application/json"   -d "{\"name\": \"${SLUG}.coreflux.studio\"}" > /dev/null 2>&1 || true
